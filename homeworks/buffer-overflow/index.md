...
<em>Note</em>: `runbin.sh` is a shell script that is just a wrapper around the following code:
...
To exploit the program, you will have to learn some information about how it is laid out in memory. You can find out this information using the `gdb` program debugger. You can <em>attach</em> `gdb` to your running program, and then use it to print information about the state of that program, and step through executions of that program. 
...
The `-p` option to `gdb` tells it to attach to a running program with the PID given to the option. The command `pgrep wisdom-alt` searches the process table to find the PID of the `wisdom-alt` program; this PID is then fed as the argument to `-p`. <em>Be warned</em>: If you have multiple `wisdom-alt` programs running, you may not attach to the one you expect! Make sure they are all killed (perhaps by killing and restarting the terminals you started them in) if you run into trouble.
